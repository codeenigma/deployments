...
if [ $? -eq 0 ]; then
  ...
  #
  #########################################################################
  if [ -f $SITE_ROOT/www/sites/default/default.settings.php ]; then
    cp $SITE_ROOT/www/sites/default/default.settings.php $SITE_ROOT/www/sites/default/settings.php
  else
    echo "Missing default.settings.php. Aborting!"
    exit 1
  fi

  if [ -d $SITE_ROOT/www/core ]; then
    cat >> $SITE_ROOT/www/sites/default/settings.php <<EOF
...
EOF
  elif [ -f $SITE_ROOT/www/modules/overlay/overlay.info ]; then
    cat >> $SITE_ROOT/www/sites/default/settings.php <<EOF
\$databases['default']['default'] = array(
 'driver' => 'mysql',
 'database' => '$NEWDB',
