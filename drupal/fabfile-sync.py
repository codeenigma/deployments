...
@task
def main(shortname, staging_branch, prod_branch, synctype='both', fresh_database='no', sanitise='yes', sanitised_password=None, sanitised_email=None, staging_shortname=None):
    ...
    if config.has_section(shortname):
      try:
        ...
        if synctype == 'files' or synctype == 'both':
          Sync.sync_assets(orig_host, staging_shortname, staging_branch, prod_branch, config)
