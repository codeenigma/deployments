...
def main(repo, repourl, build, branch, buildtype, url=None, profile="minimal", keepbuilds=10, runcron="False", doupdates="Yes", freshdatabase="Yes", syncbranch=None, sanitise="no", statuscakekey=None, statuscakeid=None, importconfig="yes", restartvarnish="yes", cluster=False):
  ...
  if fresh_install == True:
    ...
    if freshdatabase == "Yes" and buildtype == "custombranch":
      ...
      Drupal.prepare_database(repo, branch, build, syncbranch, env.host_string, sanitise, drupal_version)
    ...
    execute(InitialBuild.initial_build, repo, url, branch, build, profile, buildtype, sanitise, config, drupal_version, cluster)
    ...
    if drupal_version != '8':
      importconfig = "no"
    if freshdatabase == "Yes" and buildtype == "custombranch":
      Drupal.prepare_database(repo, branch, build, syncbranch, env.host_string, sanitise, drupal_version, False)
    execute(AdjustConfiguration.adjust_settings_php, repo, branch, build, buildtype)
    execute(AdjustConfiguration.adjust_drushrc_php, repo, branch, build)
    execute(Drupal.environment_indicator, repo, branch, build, buildtype, drupal_version)
